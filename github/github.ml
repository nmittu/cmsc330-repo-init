...
let rec poll_token device_code interval =
  ...
  if response_code <> 200 || json |> member "error" |> to_string_option |> Option.is_some
  then poll_token device_code interval
  else json |> member "access_token" |> to_string |> Lwt.return
;;
...
let get_new_auth () =
  let auth_lwt =
    ...
    let* _, body =
      Client.post
        ~headers:(Cohttp.Header.of_list [ "Accept", "application/json" ])
        ~body:(Cohttp_lwt.Body.of_string ("client_id=" ^ client_id ^ "&scope=" ^ scope))
        (Uri.of_string device_code_url)
    in
    ...
    let* _ =
      Lwt_io.printf
        "Open %s in your browser and enter the folowing code:\n %s\n"
        verification_url
        user_code
    in
...
let save_token tok = Out_channel.write_all config_file ~data:tok

let read_tok () =
  if Sys_unix.file_exists_exn config_file
  then Some (In_channel.read_all config_file)
  else None
;;
...
let get_repo_key t =
  let url =
    Printf.sprintf
      "https://api.github.com/repos/%s/%s/actions/secrets/public-key"
      t.owner
      t.repo
  in
  let* _, body =
  ...
  let json = Yojson.Basic.from_string body_string in
  let open Yojson.Basic.Util in
  (json |> member "key" |> to_string, json |> member "key_id" |> to_string) |> Lwt.return
;;
...
let set_secret t ~key ~value =
  let lwt =
    ...
    let c =
      Sealed_box.seal (Sealed_box.bytes_of_string value) (Sealed_box.key_of_string pk)
    in
        ...
        ~body:
          (Cohttp_lwt.Body.of_string
             (Yojson.Basic.to_string
                (`Assoc [ "encrypted_value", `String encoded; "key_id", `String key_id ])))
        (Uri.of_string url)
    in
    let response_code = resp |> Response.status |> Cohttp.Code.code_of_status in
    Lwt.return (response_code = 201 || response_code = 204)
  in
  Lwt_main.run lwt
;;
