...
let seal m pk =
  ...
  let boxed = Box.box ~nonce ~receiver:pk ~sender:esk ~plain_text:m in
  set c boxed Box.Public_key.length;
  c
;;

let bytes_of_string s = Bytes.of_string s
let base64_of_bytes b = Base64.encode_exn (Bytes.to_string b)
let key_of_string s = Box.Public_key.of_bytes (bytes_of_string (Base64.decode_exn s))
